...
no console:
*/
text = text.replace(/(Manuel Marques de Sousa)/g, 'Jael Araripe Rabelo');
...
*/
text = text.replace(/(brasileiro)/g, 'fortalezense');
...
console.log( '\nTrocando números por -:',  text);

/*
Substitua todas as letras (somente letras) de "D" maiúsculo até "h"
minúsculo por "0" (número zero). Mostre o resultado no console:
*/
text = text.replace(/(D-H)(d-h)/g, '0')
console.log( '\nTrocando de "D" a "h" por "0":', text );
...
console.log( '\n"O Centauro de Luvas" em caixa alta:', text.replace(/(O Centauro de Luvas)/, function(letras){
...
var ragexDate = /(?<day>[0-9]{2})\sde\s(?<month>[a-z]{4,9})\sde\s(?<year>[0-9]{4})/gi;
...
text = text.replace(ragexDate, replaceDate);
