...
var regexFormatCPF = /(\d{3})(\d{3})(\d{3})(\d{2})/;
function formatCPF(regex, cpf1, cpf2, cpf3, cpf4){
    return cpf1+'.'+cpf2+'.'+cpf3+'-'+cpf4;
};
cpf1 = cpf1.replace(regexFormatCPF, formatCPF);
cpf2 = cpf2.replace(regexFormatCPF, formatCPF);
cpf3 = cpf3.replace(regexFormatCPF, formatCPF);
cpf4 = cpf4.replace(regexFormatCPF, formatCPF);
console.log(cpf1);
console.log(cpf2);
console.log(cpf3);
console.log(cpf4);
...
var stringMatch = 'Os meses de janeiro, junho e julho começam com a letra j';
var matchArr = stringMatch.match(/(ju\w{3})/gi);
...
var stringMatch2 = '<div><section><blockquote>Texto <img /></blockquote></section></div>';
var matchArr2 = stringMatch2.match(/<\w+>/gi);
...
var stringMatch3 = '<div><ul><li></li><li></li><li><span></span></li></ul></div>';
var matchArr3 = stringMatch3.match(/(<\w+><\/\w+>)/gi);
...
var stringReplace = '<h1>Título da página</h1><p>Este é um parágrafo</p><footer>Rodapé</footer>';
var regexReplace = /<(\w+)>([^<]+)<\/\w+>/gi;
var replaceArr = stringReplace.match(regexReplace);
console.log( replaceArr);
stringReplace = stringReplace.replace(regexReplace, '<$1>O texto dentro da tag "$1" é "$2"</$1>\n');
